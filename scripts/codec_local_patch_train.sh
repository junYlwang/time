...
CPU=$((GPU * 8))
MEMORY=$((GPU * 80000))
rjob delete codec-local-patch
rjob submit \
  --name=codec-local-patch \
  --gpu=$GPU \
  --cpu=$CPU \
  --memory=$MEMORY \
...
conda activate time
torchrun --nproc_per_node=2 /mnt/shared-storage-user/wangjunyi/time/tools/codec_prediction_patch_local_train.py \
--config /mnt/shared-storage-user/wangjunyi/time/configs/codec-prediction-patch-local-fsq-data-v1.yaml
'