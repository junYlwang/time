...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29501 \
  tools/llm_train_patch_causal.py \
  --config configs/llm-train-patch-causal.yaml &
pid1=$!

CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29502 \
  tools/llm_train_patch_local.py \
  --config configs/llm-train-patch-local.yaml &
pid2=$!
