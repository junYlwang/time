...
CPU=$((GPU * 8))
MEMORY=$((GPU * 80000))
rjob delete codec-patch-causal-mae-v4-rvq
rjob submit \
  --name=codec-patch-causal-mae-v4-rvq \
  --gpu=$GPU \
  --cpu=$CPU \
  --memory=$MEMORY \
...
conda activate time
torchrun --nproc_per_node=4 /mnt/shared-storage-user/wangjunyi/time/tools/codec_patch_causal_mae_quant_train.py \
--config /mnt/shared-storage-user/wangjunyi/time/configs/codec-patch-causal-mae-v4.yaml
'