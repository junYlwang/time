...
CPU=$((GPU * 8))
MEMORY=$((GPU * 80000))
rjob delete ts-llm-patch-causal
rjob submit \
  --name=ts-llm-patch-causal \
  --gpu=$GPU \
  --cpu=$CPU \
  --memory=$MEMORY \
...
conda activate time
torchrun --nproc_per_node=4 /mnt/shared-storage-user/wangjunyi/time/tools/llm_train_patch_causal.py \
--config /mnt/shared-storage-user/wangjunyi/time/configs/llm-train-patch-causal.yaml \
'