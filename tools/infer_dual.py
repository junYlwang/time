...
from modules.utils import AttrDict, load_checkpoint, load_hparams
...
def _load_codec_checkpoint(h, device: torch.device):
    ...
    encoder_trend.load_state_dict(state["encoder_trend"], strict=True)
    quantizer_trend.load_state_dict(state["quantizer_trend"], strict=True)
    decoder_trend.load_state_dict(state["decoder_trend"], strict=True)
    encoder_residual.load_state_dict(state["encoder_residual"], strict=True)
    quantizer_residual.load_state_dict(state["quantizer_residual"], strict=True)
    decoder_residual.load_state_dict(state["decoder_residual"], strict=True)
    if "input_norm" in state:
        input_norm.load_state_dict(state["input_norm"], strict=True)
    if "decomposition" in state:
        decomposition.load_state_dict(state["decomposition"], strict=True)

    for module in models:
        module.eval()
    _set_quantizer_eval_mode(quantizer_trend)
    _set_quantizer_eval_mode(quantizer_residual)
...
def _infer_one_subset(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            trend_hat_norm = decoder_trend(quantizer_trend(encoder_trend(trend_target_norm)).z_q)
            residual_hat_norm = decoder_residual(quantizer_residual(encoder_residual(residual_target_norm)).z_q)
...
def main() -> None:
    ...
    (
        ...
        decomposition,
        checkpoint_path,
    ) = _load_codec_checkpoint(h, device)
    ...
    for subset_root in test_roots:
        ...
        res = _infer_one_subset(
            ...
            quantizer_trend=quantizer_trend,
            decoder_trend=decoder_trend,
            encoder_residual=encoder_residual,
            quantizer_residual=quantizer_residual,
